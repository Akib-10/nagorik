...
const STEP_TITLES = {
  1: [
    "Report an Issue",
    "Fill in the details below — our city team will review it shortly.",
  ],
  2: [
    "Add Location & Photo",
    "Drop a pin on the map and attach photos of the issue.",
  ],
...
export default function ReportIssue() {
  ...
  useEffect(() => {
    if (!editId) return;
    findReport(editId).then((report) => {
      if (!report) return;
      setEditing(report);
      setTitle(report.title || "");
      setCategory(report.category || "Roads & Infrastructure");
      setPriority(report.priority || "Medium");
      setArea(report.area || "");
      setDate(report.date || "");
      setDescription(report.description || "");
      setFullAddress(report.address || "");
      const photos = report.photos || [];
      setSlots([photos[0] || null, photos[1] || null, photos[2] || null]);
      setCoordsText(report.coordsText || DEFAULT_COORDS);
      setLoaded(true);
    });
  }, [editId]);
  ...
  const handleMapClick = (e) => {
    ...
    const lat = (23.9 - (yPct / 100) * 0.22).toFixed(4);
    const lng = (90.28 + (xPct / 100) * 0.3).toFixed(4);
  ...
  const chipClass = /high/i.test(priority)
    ? "bg-nagorik-red text-white"
    : /low/i.test(priority)
      ? "bg-[#E8F5EE] text-nagorik-green"
      : "";
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      if (editing) {
        await updateReport(editing._id, reportData);
  ...
  if (!loaded) return <p className="p-10 text-center">Loading...</p>;
  return (
    <>
      <AppHeader
        logoHref="/browse_feed"
        navItems={[{ label: "HOME", to: "/browse_feed", icon: <HomeGlyph /> }]}
        showIconButtons
      />

      <main className="mx-auto max-w-[1160px] px-7 max-[760px]:px-4">
        <div className="py-[36px_0_28px] text-center">
          <h1 className="mb-2 m-0 text-[30px] font-extrabold text-nagorik-heading">
            {STEP_TITLES[currentStep][0]}
          </h1>
          <p className="m-0 text-[14px] text-nagorik-secondary">
        ...
        <div className="mb-6 flex flex-wrap gap-3.5">
          {[1, 2, 3].map((s) => {
            ...
            return (
              ...
              >
                <span
                  className={`flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-full text-[11px] font-extrabold ${isActive ? "bg-white text-nagorik-red" : isDone ? "bg-nagorik-red text-white" : "bg-white text-nagorik-red"}`}
                >
                  {isDone ? "✓" : s}
                </span>
                {
                  ["Issue Details", "Add Location & Photo", "Review & Submit"][
                    s - 1
                  ]
                }
              </button>
        ...
        <section className={currentStep === 1 ? "block" : "hidden"}>
          <form
            className="mb-[22px] flex flex-col gap-5 rounded-[18px] border border-nagorik-border bg-nagorik-surface p-[26px]"
            id="report-step1"
          ...
          >
            <div className="grid grid-cols-3 gap-[18px] max-[900px]:grid-cols-1">
              <div className="flex flex-col">
                <label
                  htmlFor="issueTitle"
                  className="mb-2 block text-[12.5px] font-bold text-nagorik-heading"
                >
                  Issue Title *
                </label>
                <input
            ...
            <div className="grid grid-cols-2 gap-[18px] max-[900px]:grid-cols-1">
              <div className="flex flex-col">
                ...
                <input
                  id="issueArea"
                  type="text"
                  placeholder="e.g. Mirpur 10, near Shadhinota Complex"
                  required
                  value={area}
            ...
            <div className="flex flex-col">
              ...
              <textarea
                id="issueDesc"
                placeholder="Describe the problem in detail — what is wrong, since when, and how it affects people nearby..."
                required
                value={description}
          ...
          <div className="flex justify-center pb-10">
            <button
              type="submit"
              form="report-step1"
              className="rounded-full bg-nagorik-red px-[64px] py-[18px] text-[15px] font-bold text-white transition-colors duration-150 hover:bg-nagorik-hover-red max-[900px]:w-full"
            >
              Next
            </button>
          </div>
        ...
        <section className={currentStep === 2 ? "block" : "hidden"}>
          ...
          <div className="mb-[22px] grid grid-cols-2 gap-[22px] max-[900px]:grid-cols-1">
            <div className="flex flex-col rounded-[18px] border border-nagorik-border bg-nagorik-surface p-[22px]">
              <h3 className="m-0 mb-1 text-[15px] font-extrabold text-nagorik-heading">
                Upload Photos
              </h3>
              <p className="sub m-0 mb-4 text-[12px] text-nagorik-muted">
                Add up to 3 photos — clear daylight photos help faster
                resolution.
              </p>
              <div className="mt-4 grid flex-1 grid-cols-3 gap-3">
                {[0, 1].map((index) => (
                  <button
                    type="button"
                    key={index}
                    className={`relative flex min-h-[170px] cursor-pointer flex-col items-center justify-center gap-2 overflow-hidden rounded-[14px] border-2 border-dashed ${slots[index] ? "border-nagorik-light-red bg-nagorik-soft-red" : "border-nagorik-light-red bg-nagorik-soft-red"} p-2.5 text-[11.5px] font-semibold text-nagorik-muted font-[inherit] transition-colors duration-150 hover:border-nagorik-red`}
                  ...
                  >
                    {slots[index] ? (
                      <>
                        ...
                        <button
                          type="button"
                          className="absolute right-2 top-2 z-[2] flex h-[26px] w-[26px] items-center justify-center rounded-full bg-nagorik-red/92 text-[14px] leading-none text-white cursor-pointer"
                ...
                <button
                  type="button"
                  className="relative flex min-h-[170px] cursor-pointer flex-col items-center justify-center gap-2 overflow-hidden rounded-[14px] border-2 border-dashed border-nagorik-light-red bg-nagorik-soft-red p-2.5 text-[11.5px] font-semibold text-nagorik-muted font-[inherit] transition-colors duration-150 hover:border-nagorik-red"
                ...
                >
                  {slots[2] ? (
                    ...
                    <span className="text-[40px] font-extralight leading-none text-nagorik-red">
                      +
                    </span>
                  )}
                </button>
            ...
            <div className="flex flex-col rounded-[18px] border border-nagorik-border bg-nagorik-surface p-[22px]">
              ...
              <button
                type="button"
                className="flex w-fit items-center gap-2 rounded-full bg-nagorik-red px-5 py-[11px] text-[12.5px] font-bold text-white transition-colors duration-150 hover:bg-nagorik-hover-red font-[inherit] max-[900px]:w-full max-[900px]:justify-center"
          ...
          <section className="mb-[22px] rounded-[18px] border border-nagorik-border bg-nagorik-surface p-[18px]">
            <p className="m-0 mb-2.5 ml-1 text-[12.5px] font-bold text-nagorik-heading">
              Pin the exact spot — tap anywhere on the map
            </p>
            <div
            ...
            >
              ...
              >
                <path
                  d="M17 1C8.7 1 2 7.7 2 16c0 10.5 15 27 15 27s15-16.5 15-27C32 7.7 25.3 1 17 1z"
                  fill="#C8102E"
                />
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pb-10">
            <button
              type="button"
              className="inline-flex items-center justify-center gap-[9px] rounded-full border-[1.5px] border-nagorik-border bg-white px-[56px] py-[17px] text-[15px] font-bold text-nagorik-secondary transition-[border-color_0.15s_ease,color_0.15s_ease] hover:border-nagorik-red hover:text-nagorik-red dark:bg-nagorik-surface"
            ...
            <button
              type="submit"
              form="report-step2"
              className="rounded-full bg-nagorik-red px-[64px] py-[18px] text-[15px] font-bold text-white transition-colors duration-150 hover:bg-nagorik-hover-red max-[900px]:w-full max-[900px]:justify-center"
            >
              Next
            </button>
          </div>
        ...
        <section className={currentStep === 3 ? "block" : "hidden"}>
          <div className="mb-[22px] rounded-[18px] border border-nagorik-border bg-nagorik-surface p-[26px]">
            <div className="mb-2 flex items-start justify-between gap-4">
              <div>
                <h3 className="m-0 mb-1 text-[16px] font-extrabold text-nagorik-heading">
                  Report Summary
                </h3>
                <p className="sub m-0 text-[12px] text-nagorik-muted">
                  Please double-check everything before submitting.
                </p>
              </div>
              <span
                className={`whitespace-nowrap rounded-full px-[18px] py-2 text-[12px] font-extrabold ${chipClass || "bg-nagorik-soft-red text-nagorik-red"}`}
              >
                {priority}
              </span>
            </div>
            ...
            <div>
              {reviewRows.map(([k, v]) => (
                <div
                  className="flex gap-[24px] border-b border-nagorik-cream py-[13px] text-[13.5px]"
                  key={k}
                >
                  <span className="w-[190px] shrink-0 text-nagorik-muted font-semibold">
                    {k}
                  </span>
                  <span className="break-words text-nagorik-heading font-bold text-right">
                    {v || "—"}
                  </span>
                </div>
              ))}
            ...
            <h4 className="mt-5 mb-3 text-[13px] font-extrabold text-nagorik-heading">
              Attached Photos
            </h4>
              ...
              {filledPhotos.length ? (
                filledPhotos.map((src, i) => (
                  <img
                    ...
                    src={src}
                    alt="attached photo"
                    className="h-[100px] w-[130px] rounded-[10px] border border-nagorik-border object-cover"
                  />
                ))
              ) : (
            ...
            <div className="mt-5 flex items-center gap-3 rounded-xl bg-nagorik-soft-red px-[18px] py-3.5 text-nagorik-red">
              ...
              <div>
                ...
                <span className="text-[12.5px] text-nagorik-secondary">
                  {coordsText}
                </span>
              </div>
            </div>
